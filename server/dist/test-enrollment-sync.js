...
async function testEnrollmentSync() {
    ...
    const student = await prisma_1.prisma.user.findUnique({ where: { email: STUDENT_EMAIL } });
    const course = await prisma_1.prisma.course.findUnique({ where: { id: COURSE_ID } });
    if (!student || !course) {
    ...
    const allStudentEnrollments = await prisma_1.prisma.enrollment.findMany({
    ...
    const enrollmentsQuery = await prisma_1.prisma.enrollment.findMany({
        where: { userId: student.id },
        include: {
            course: {
                include: {
