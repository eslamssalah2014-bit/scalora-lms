...
async function runForensicPushAudit() {
    ...
    console.log(`- VAPID Public Key: ${webpush_service_js_1.VAPID_PUBLIC_KEY ? webpush_service_js_1.VAPID_PUBLIC_KEY.slice(0, 20) + '...' : 'MISSING'}`);
    console.log(`- VAPID Private Key: ${webpush_service_js_1.VAPID_PRIVATE_KEY ? 'PRESENT (Valid Length: ' + webpush_service_js_1.VAPID_PRIVATE_KEY.length + ' chars)' : 'MISSING'}`);
    console.log(`- VAPID Subject: ${webpush_service_js_1.VAPID_SUBJECT}`);
    try {
        web_push_1.default.setVapidDetails(webpush_service_js_1.VAPID_SUBJECT, webpush_service_js_1.VAPID_PUBLIC_KEY, webpush_service_js_1.VAPID_PRIVATE_KEY);
    ...
    const storageFile = path_1.default.join(dataDir, 'push_subscriptions.json');
    console.log(`- Storage Path: ${storageFile}`);
    if (fs_1.default.existsSync(storageFile)) {
        const raw = fs_1.default.readFileSync(storageFile, 'utf-8');
    ...
    const swPath = path_1.default.resolve(process.cwd(), '../client/public/sw.js');
    if (fs_1.default.existsSync(swPath)) {
        const swContent = fs_1.default.readFileSync(swPath, 'utf-8');
    ...
    const pushLibPath = path_1.default.resolve(process.cwd(), '../client/src/lib/pushNotifications.ts');
    if (fs_1.default.existsSync(pushLibPath)) {
        const pushLibContent = fs_1.default.readFileSync(pushLibPath, 'utf-8');
