...
exports.DEFAULT_TRAINER_POLICY = exports.INITIAL_TRACKS = void 0;
exports.seedTrainerDefaults = seedTrainerDefaults;
const prisma_js_1 = require("../lib/prisma.js");
exports.INITIAL_TRACKS = [
...
exports.DEFAULT_TRAINER_POLICY = `# Scalora Trainer & Academic Standards Policy
...
async function seedTrainerDefaults() {
    console.log('🌱 Checking and seeding default Teaching Tracks...');
    for (const track of exports.INITIAL_TRACKS) {
        const existing = await prisma_js_1.prisma.teachingTrack.findUnique({
            where: { slug: track.slug },
        });
        if (!existing) {
            await prisma_js_1.prisma.teachingTrack.create({
                data: track,
            });
    ...
    const existingPolicy = await prisma_js_1.prisma.trainerPolicy.findFirst({
        where: { isActive: true },
    });
    if (!existingPolicy) {
        const policy = await prisma_js_1.prisma.trainerPolicy.create({
            data: {
                title: 'Scalora Trainer & Academic Standards Policy',
                content: exports.DEFAULT_TRAINER_POLICY,
                version: 1,
                isActive: true,
