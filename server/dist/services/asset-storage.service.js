...
class AssetStorageService {
    static uploadsBaseDir = path_1.default.join(process.cwd(), 'uploads');
    /**
     ...
     */
    static async saveAsset(options) {
        ...
        if (!safeFileName || safeFileName.trim().length === 0) {
            ...
            const parsed = path_1.default.parse(safeFileName);
        ...
        const folderDir = path_1.default.join(this.uploadsBaseDir, cleanFolder);
        this.ensureDir(folderDir);
        const filePath = path_1.default.join(folderDir, safeFileName);
        ...
        const assetRecord = await prisma_js_1.prisma.storedAsset.upsert({
     ...
     */
    static async getAsset(folder, fileName) {
        const cleanFolder = (folder || 'general').replace(/[^a-zA-Z0-9_-]/g, '');
        const cleanFileName = path_1.default.basename(fileName);
        // 1. Check disk cache
        const targetPath = path_1.default.join(this.uploadsBaseDir, cleanFolder, cleanFileName);
        if (fs_1.default.existsSync(targetPath)) {
            try {
                const buffer = fs_1.default.readFileSync(targetPath);
        ...
        const fallbackPath = path_1.default.join(this.uploadsBaseDir, cleanFileName);
        if (fs_1.default.existsSync(fallbackPath)) {
            try {
                const buffer = fs_1.default.readFileSync(fallbackPath);
                const mimeType = this.detectMimeType(cleanFileName);
                return { buffer, mimeType };
            }
            catch (err) {
        ...
        try {
            const asset = await prisma_js_1.prisma.storedAsset.findFirst({
                where: {
            ...
            if (asset && asset.dataBase64) {
                ...
                try {
                    this.ensureDir(path_1.default.dirname(targetPath));
     ...
     */
    static getPlaceholderSvg(category = 'course', title = 'Scalora LMS') {
