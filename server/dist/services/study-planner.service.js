...
class StudyPlannerService {
    storageFile;
    plans = new Map(); // key: `${userId}_${courseId}`
    constructor() {
        ...
        this.storageFile = path_1.default.join(dataDir, 'study_plans.json');
        this.loadData();
    }
    loadData() {
        try {
    ...
    parseDurationMinutes(duration) {
        if (!duration)
            return 12; // Standard default 12 minutes per lesson if unspecified
     ...
     */
    async createOrUpdatePlan(userId, courseId, targetDays, customTargetDate) {
     ...
     */
    async getPlanMetrics(userId, courseId) {
     ...
     */
    async getAllPlansForStudent(userId) {
     ...
     */
    async getAnalytics() {
        const allCourses = await prisma_js_1.prisma.course.findMany({
        ...
        for (const [key, plan] of this.plans.entries()) {
            try {
                ...
                if (metrics) {
                    ...
                    if (metrics.scheduleStatus === 'ON_TRACK')
                        studentsOnTrack++;
     ...
     */
    async sendDailyStudyReminders() {
        let delivered = 0;
        for (const [key, plan] of this.plans.entries()) {
            try {
                const metrics = await this.getPlanMetrics(plan.userId, plan.courseId);
                if (!metrics || metrics.completionPercent >= 100)
                    continue;
                ...
                await webpush_service_js_1.webPushService.sendPushToUser(plan.userId, {
                    title,
                    body,
...
exports.studyPlannerService = new StudyPlannerService();
