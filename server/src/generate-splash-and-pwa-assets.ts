...
function findTightBoundingBox(src: PNG): BoundingBox {
  let minX = src.width;
  let maxX = 0;
  let minY = src.height;
  let maxY = 0;
...
async function generateAllPwaAssets() {
  ...
  fs.writeFileSync(path.join(publicDir, 'pwa-192x192.png'), PNG.sync.write(pwa192));
  ...
  fs.writeFileSync(path.join(publicDir, 'pwa-512x512.png'), PNG.sync.write(pwa512));
  ...
  fs.writeFileSync(path.join(publicDir, 'pwa-512x512-maskable.png'), PNG.sync.write(pwaMaskable));
  ...
  fs.writeFileSync(path.join(publicDir, 'apple-touch-icon.png'), PNG.sync.write(appleTouch));
  ...
  fs.writeFileSync(path.join(publicDir, 'scalora-icon-transparent.png'), PNG.sync.write(scaloraIconTrans));
  console.log('✓ Generated scalora-icon-transparent.png (Transparent)');

  // 6. scalora-logo.png & logo.png (100% Transparent)
  fs.writeFileSync(path.join(publicDir, 'scalora-logo.png'), PNG.sync.write(pwa512));
  fs.writeFileSync(path.join(publicDir, 'logo.png'), PNG.sync.write(pwa512));
  ...
  fs.writeFileSync(path.join(iconsDir, 'notification-icon-192.png'), PNG.sync.write(notif192));
  fs.writeFileSync(path.join(iconsDir, 'notification-icon-96.png'), PNG.sync.write(notif96));
  ...
  const badgePng = new PNG({ width: 96, height: 96 });
  ...
  fs.writeFileSync(path.join(iconsDir, 'badge-icon.png'), PNG.sync.write(badgePng));
