...
function runKashierVerificationTests() {
  ...
  const generatedHash = kashierService.generateOrderHash({
    orderId: testOrderId,
    amount: testAmount,
  ...
  const manualHash = crypto.createHmac('sha256', testSecret).update(expectedPath).digest('hex');
  ...
  const callbackSignature = crypto.createHmac('sha256', testSecret).update(queryStr).digest('hex');
  callbackParams.signature = callbackSignature;

  const isCallbackValid = kashierService.verifyCallbackSignature(callbackParams);
  ...
  const isTamperedValid = kashierService.verifyCallbackSignature(tamperedParams);
  ...
  const dataToSign = sortedKeys
    .map((k) => `${k}=${encodeURIComponent((webhookPayload as any)[k])}`)
    .join('&');

  const webhookSignature = crypto.createHmac('sha256', testSecret).update(dataToSign).digest('hex');
  const rawBody = Buffer.from(JSON.stringify(webhookPayload));

  const isWebhookValid = kashierService.verifyWebhookSignature(rawBody, webhookSignature);
