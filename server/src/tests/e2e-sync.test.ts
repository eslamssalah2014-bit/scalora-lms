...
function record(step: string, name: string, passed: boolean, details: string, data?: any) {
...
async function runFullE2ETestSuite() {
  ...
  try {
    ...
    record(
      'DB_AUDIT',
      'Verify Supabase Database Connectivity & Seeded Data',
      dbHealthy,
      `Connected to Supabase PostgreSQL. Found ${existingUsers} users, ${existingCourses} courses, ${existingModules} modules, ${existingLessons} lessons.`
    );

    // Get Admin user & a Target Course
    const adminUser = await prisma.user.findFirst({ where: { role: 'ADMIN' } });
    if (!adminUser) throw new Error('No admin user found in database seed!');
    ...
    if (!firstCourse) throw new Error('No published course found in database seed!');
    ...
    record(
      'REGISTRATION',
      'Create Student & Persist to Supabase Users Table',
      regPassed,
      `User ID: ${createdUserId}, Email: ${testEmail}, Persisted in Supabase: ${regPassed}`
    );
    ...
    // Test Wrong Password
    const wrongPasswordValid = await bcrypt.compare('WrongPassword999', verifiedStudent!.password);
    record(
      'AUTH_WRONG_PW',
      'Reject Invalid Password Attempt',
      !wrongPasswordValid,
      'Authentication correctly rejects incorrect password.'
    );

    // Test Correct Password
    const correctPasswordValid = await bcrypt.compare(testPassword, verifiedStudent!.password);
    studentToken = jwt.sign(
      { id: verifiedStudent!.id, email: verifiedStudent!.email, role: verifiedStudent!.role, name: verifiedStudent!.name },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
    record(
      'AUTH_LOGIN',
      'Generate Valid JWT Session for Student',
      correctPasswordValid && !!studentToken,
      `Token generated for ${testEmail}`
    );
    ...
    record(
      'COURSE_DISCOVERY',
      'Load Real Course, Modules, and Lessons from Supabase',
      courseDiscoveryPassed,
      `Course: "${dbCourse?.title}" (${dbCourse?.modules.length} modules, ${dbCourse?.modules[0].lessons.length} lessons)`
    );
    ...
    record(
      'ENROLLMENT_CREATE',
      'Persist Enrollment Record in Supabase PostgreSQL',
      !!enrollmentPersisted,
      `Enrollment ID: ${newEnrollment.id}, Student: ${createdUserId}, Course: ${targetCourseId}`
    );
    ...
    record(
      ...
      'Prevent Duplicate Course Enrollment in Database',
      duplicatePrevented,
      'PostgreSQL unique constraint [userId, courseId] successfully blocked duplicate enrollment.'
    );
    ...
    record(
      'ADMIN_VISIBILITY',
      'Admin Real-Time Visibility of Student Enrollment',
      adminSeesEnrollment,
      `Admin queried enrollment for ${testEmail} in "${adminEnrollments[0]?.course?.title}".`
    );
    ...
    if (targetLessonId) {
      ...
      record(
        'LESSON_PROGRESS',
        'Save & Persist Lesson Completion in Supabase',
        !!verifiedProgress && verifiedProgress.isCompleted,
        `Lesson ID: ${targetLessonId}, Completed: ${verifiedProgress?.isCompleted}`
      );
    }
    ...
    if (targetQuizId) {
      ...
      if (quiz && quiz.questions.length > 0) {
        ...
        record(
          'QUIZ_RESULT',
          'Persist Quiz Attempt & Score in Supabase PostgreSQL',
          !!verifiedResult && verifiedResult.score === 100.0 && verifiedResult.passed,
          `Quiz: "${quiz.title}", Score: ${verifiedResult?.score}%, Passed: ${verifiedResult?.passed}`
        );
      }
    }
    ...
    record(
      'CERTIFICATE',
      'Generate & Verify Unique Certificate in Supabase',
      !!verifiedCert && verifiedCert.certificateNumber === certNumber,
      `Certificate #: ${certNumber}, Student: ${verifiedCert?.studentName}, Course: ${verifiedCert?.courseTitle}`
    );
    ...
    record(
      'ADMIN_COURSE_CREATE',
      'Admin Create Course with Modules & Lessons in Supabase',
      !!verifiedAdminCourse && verifiedAdminCourse.modules.length > 0,
      `Course ID: ${createdCourse.id}, Modules: ${verifiedAdminCourse?.modules.length}, Lessons: ${verifiedAdminCourse?.modules[0].lessons.length}`
    );
