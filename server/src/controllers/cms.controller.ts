...
export const DEFAULT_CMS_DATA: Record<string, any> = {
  page_home: {
    hero: {
...
export const getPublishedDocument = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.json({
      success: true,
      key,
      data: DEFAULT_CMS_DATA[key] || {},
      isDefault: true,
    });
  }
};

export const getAllPublished = async (_req: Request, res: Response): Promise<void> => {
  try {
    const docs = await prisma.cmsDocument.findMany();
    const result: Record<string, any> = { ...DEFAULT_CMS_DATA };
...
export const getTheme = async (_req: Request, res: Response): Promise<void> => {
  try {
    ...
    let themeData = DEFAULT_CMS_DATA.theme;
    if (doc?.publishedData) {
      try {
...
export const getSitemapXml = async (_req: Request, res: Response): Promise<void> => {
  try {
    const seoDoc = await prisma.cmsDocument.findUnique({ where: { key: 'seo' } });
    ...
    if (seoDoc?.publishedData) {
      try {
        ...
        if (parsed.canonicalDomain) canonicalDomain = parsed.canonicalDomain.replace(/\/$/, '');
      } catch {}
    }
...
export const getAdminDocuments = async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const docs = await (prisma.cmsDocument.findMany as any)({
      include: {
        revisions: {
    ...
    const keys = Object.keys(DEFAULT_CMS_DATA);
    const existingMap = new Map((docs as any[]).map((d) => [d.key, d]));
    ...
    const result = keys.map((k) => {
      ...
      return {
        ...
        hasDraftChanges: Boolean(
          existing && existing.publishedData && existing.draftData !== existing.publishedData
        ),
...
export const getAdminDocumentByKey = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
    const defaultData = DEFAULT_CMS_DATA[key] || {};

    let doc: any = await (prisma.cmsDocument.findUnique as any)({
      where: { key },
      include: {
        revisions: {
          orderBy: { createdAt: 'desc' },
    ...
    if (!doc) {
      doc = await (prisma.cmsDocument.create as any)({
        data: {
          key,
    ...
    try {
      if (doc.publishedData) publishedData = JSON.parse(doc.publishedData);
    } catch {}
...
export const saveAdminDraft = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
    const { data } = req.body;
    ...
    const doc = await prisma.cmsDocument.upsert({
      where: { key },
      update: {
      ...
      create: {
        key,
        draftData: draftDataString,
        publishedData: JSON.stringify(DEFAULT_CMS_DATA[key] || data),
        isPublished: false,
        lastUpdatedBy: userName,
      },
    });
...
export const publishAdminDraft = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
    const { note } = req.body;
    const userName = req.user?.name || 'Admin';

    const doc: any = await (prisma.cmsDocument.findUnique as any)({
      where: { key },
      include: {
        revisions: {
    ...
    const [updatedDoc, newRevision] = await prisma.$transaction([
...
export const revertAdminDraft = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
    const doc = await prisma.cmsDocument.findUnique({
      where: { key },
...
export const restoreAdminRevision = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
...
export const resetAdminToDefaults = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  const key = getParam(req.params.key);
  try {
    const defaultData = DEFAULT_CMS_DATA[key];
    ...
    const updated = await prisma.cmsDocument.upsert({
      where: { key },
      update: {
        draftData: JSON.stringify(defaultData),
        lastUpdatedBy: req.user?.name || 'Admin',
...
export const getMediaLibrary = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const mediaList = await prisma.cmsMedia.findMany({
      where,
      orderBy: { createdAt: 'desc' },
      take: 100,
    });
...
export const uploadMedia = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (mimeMatch && mimeMatch[1]) {
      ...
      if (parsedExt) {
        ext = parsedExt;
        if (['svg'].includes(parsedExt)) fileType = 'SVG';
        else if (['pdf'].includes(parsedExt)) fileType = 'PDF';
        else if (['mp4', 'webm', 'mov'].includes(parsedExt)) fileType = 'VIDEO';
      }
    }
