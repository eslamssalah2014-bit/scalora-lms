...
export const toggleLessonProgress = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    const { lessonId, isCompleted } = req.body as { lessonId: string; isCompleted?: boolean };
    ...
    if (isCourseCompleted) {
      ...
      const course = await prisma.course.findUnique({ where: { id: lesson.module.courseId } });
...
export const getCertificate = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    const courseId = req.params.courseId as string;
